...
2. Check whether AI-DLC has already been set up in this repo by looking for files that are **only created during onboarding** — not files that ship with the base repo. The following are reliable indicators of an existing setup (check at `FRAMEWORK_ROOT`):
   ...
   **Do not treat the following as evidence of existing setup** — they are part of the base repo and are present in every fresh copy:
   ...
   - `process-onboarding-agent/skills/product-engineering-essentials.md`
   - `process-onboarding-agent/skills/release-readiness-checklist.md`
...
2. Write the master rule file (all 11 sections) using interview answers for Sections 1–5 and Section 9. Section 10 (Notifications) and Section 11 (AI Hub Metrics) are settled later, when each skill's setup conversation runs during **setup guide Step 4** — until then, write both as `Status: Disabled`. **Replace every `{FRAMEWORK_ROOT}` placeholder in the templates with the actual resolved path** (e.g. `docs/process/intent-execution-framework`). The master rule file must contain real paths — never the literal string `{FRAMEWORK_ROOT}`.
...
5. Run the skills that need a setup conversation of their own — copying a skill file installs the *file*, not the configuration. Currently this means the notifications skill and the ai-hub-metrics skill: run the *Onboarding setup* steps inside `{FRAMEWORK_ROOT}/skills/notifications.md` and `{FRAMEWORK_ROOT}/skills/ai-hub-metrics.md` (setup guide Step 4), then write Section 10 and Section 11 with the answers. Each skill ends its own setup with a status summary shown to the engineer — do not skip that step. If the engineer declines or is not ready for either, leave its section as `Status: Disabled` and carry on — never block onboarding on it.
...
1. Write the master rule file populated from archaeology output (Sections 1–5 from what you found; Sections 6–8 from guide defaults; Section 9 as in the guide; Sections 10 and 11 as `Status: Disabled` — each is settled by item 7 below, once that skill's setup conversation has run). **Replace every `{FRAMEWORK_ROOT}` placeholder in the templates with the actual resolved path** (e.g. `docs/process/intent-execution-framework`). The master rule file must contain real paths — never the literal string `{FRAMEWORK_ROOT}`.
